...
const defaultBaseURL = "https://accounts.spotify.com"
...
func (c *Client) endpointURL(path string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("parse spotify oauth base url: %w", err)
	}
...
func (c *Client) newRequest(ctx context.Context, method, path string, query url.Values, body io.Reader) (*http.Request, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create spotify oauth request: %w", err)
	}
...
func (c *Client) do(req *http.Request, out any) error {
	...
	if err != nil {
		return fmt.Errorf("execute spotify oauth request: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(out); err != nil {
		return fmt.Errorf("decode spotify oauth response: %w", err)
	}
